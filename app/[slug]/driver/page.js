...
export default function DriverPortal() {
  ...
  const fetchDriverTrips = async () => {
    ...
    if (data) {
      setTrips(data)
      const active = data.find(t => t.status === 'in-transit')
      const pending = data.find(t => t.status === 'pending')
      setActiveTrip(active || pending || null)
    }
    setLoading(false)
  }
  ...
  const statusBadge = (status) => {
    ...
    if (status === 'pending') return <span className="bg-purple-500/20 text-purple-400 text-xs px-2 py-1 rounded-full">⏳ Pending Accept</span>
  }
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">{driver.email || 'No email set'}</p>
                <p className="text-gray-400 text-sm">📞 {driver.phone || 'No phone set'}</p>
                <p className="text-gray-400 text-sm">🚛 Truck: <span className="font-mono text-white">{driver.truck_id || 'Not assigned'}</span></p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Pending Trip — needs driver acceptance */}
        {trips.filter(t => t.status === 'pending').map(trip => (
          <div key={trip.id} className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 mb-4">
            <p className="text-purple-400 font-semibold text-sm mb-2">⏳ New Trip Assigned — Tap to Accept</p>
        ...
        {/* Active Trip */}
        {activeTrip && activeTrip.status === 'in-transit' && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-6">
            ...
            <div className="mb-4">
              ...
              <p className="text-gray-400 text-sm">{activeTrip.litres_loaded.toLocaleString()} L loaded · {activeTrip.truck_id}</p>
              <p className="text-gray-400 text-xs mt-1">Started: {new Date(activeTrip.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Africa/Nairobi' })}</p>
            </div>
            <button
              onClick={() => handleMarkArrived(activeTrip)}
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800">
          ...
          {loading ? (
          ...
          ) : trips.filter(t => t.status !== 'in-transit' && t.status !== 'pending').length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {trips.filter(t => t.status !== 'in-transit' && t.status !== 'pending').map((trip) => (
                    <tr key={trip.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                      ...
                      <td className="p-4 text-gray-400">
                        {new Date(trip.created_at).toLocaleDateString('en-KE', { timeZone: 'Africa/Nairobi' })}
                      </td>
                    </tr>
                  ))}
                </tbody>
